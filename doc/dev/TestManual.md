...
| `KEY_ENCRYPTION_SECRET` | 测试 API Key 的加密密钥 | `test-key-encryption-secret`（Node 测试服务器为了结果稳定仍会设置；生产可省略并由服务自动生成） |
...
- 测试服务器仍会设置独立的 `KEY_ENCRYPTION_SECRET`，不要把它配成 `ROOT_TOKEN`。生产环境可不设，由服务自动生成并写入数据库
