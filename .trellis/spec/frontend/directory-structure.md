...
| `components/` | 可复用 UI，按 `common/`、`layout/` 或业务域分组 | `components/common/RecordTable.vue`、`components/layout/AppLayout.vue` |
...
| `stores/` | 跨页面共享的 Pinia 状态或资源 store | `stores/auth.ts`、`stores/record.ts`、`stores/users.ts` |
| `repositories/` | 资源读写、字段校验和本地 mock 持久化 | `repositories/mockUsers.ts`、`repositories/mockModels.ts` |
...
管理资源的数据流是 view/component → resource store → repository；记录、系统设置和连通性测试的数据流是 view/component → Pinia action 或 `api` 函数 → `utils/request` → 后端。需要可复用表格的页面应像 `views/User/List.vue` 一样，将 `useResourceTable` 与类型化 fetcher 组合，而不是重复实现分页逻辑。
